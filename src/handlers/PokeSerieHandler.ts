...
export class PokeSerieHandler {
    ...
    static async getAll(language = 'en', getSets = false): Promise<Array<PokeSerie> | null> {
        ...
        for (const serie of series) {
            ...
            if (info!=null) {
                for (const set of info.sets) {
                    pokeSets.push(new PokeSetMini(set.id, set.name, set.logo, set.symbol, set.cardCount));
                }
            }

            pokeSeries.push(new PokeSerie(serie.id, serie.name, serie.logo, pokeSets));
        }
